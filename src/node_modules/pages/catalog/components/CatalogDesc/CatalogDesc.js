...
import FilterTitle from "pages/catalog/components/Filter-title/Filter-title";
import CardSale from "components/Catalog/CardSale/CardSale";
import CATALOG from "Consts/catalog/catalog";
import Checkbox from "components/Buttons/Checkbox/Checkbox";
import SliderRange from "pages/catalog/components/Slider/SliderRange";
...
export default function CatalogDesc() {
  ...
  const productCardsToShow = CATALOG.filter((a) => a.isBanner !== true);
  const bannerToShow = CATALOG.map(
    (a) => a.isBanner && <CardSale key={a.id} />
  );

  const cardsPerPage = 11;
  const pagesVisited = (pageNumber - 1) * cardsPerPage;
  const pageCount = Math.ceil(productCardsToShow.length / cardsPerPage);

  const displayCards = productCardsToShow
    .slice(pagesVisited, pagesVisited + cardsPerPage)
    .map((product) => (
      <CardProduct {...product} key={product.id} item={product} />
    ));
      ...
      <div className="catalog-grid">
        ...
        <div className="catalog-grid__right-column">
          ...
          {/* {CATALOG.map((product) => {
            ...
            return <CardProduct {...product} key={product.id} item={product} />;
          })} */}
          {bannerToShow}
          {displayCards}
        </div>
